...
public class MazeSearch {
    ...
    public void exploreFromCellShortest(MazeCell start) {
        ...
        Map<MazeCell, MazeCell> cameFrom = new HashMap<MazeCell, MazeCell>();   // map from cell to cell it came from. e.g start came from null and neighbour came from current
        queue.add(start);
        ...
        while (current != null) {
            if (current == maze.getGoal()) { current.draw(Color.blue); }
            else { current.draw(Color.yellow); }
            UI.sleep(delay);
