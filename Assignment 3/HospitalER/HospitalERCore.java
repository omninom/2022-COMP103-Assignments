...
public class HospitalERCore{
    ...
    public void run(){
        ...
        while (running){         // each time step, check whether the simulation should pause.
            time++;                                                                     //each loop increments the time tick
            ...
            treatmentRoom.removeAll(toRemove);                                              //remove all patients that have been treated
            
            for (Patient p : waitingRoom){
                p.waitForATick();
            }
            if (treatmentRoom.size() < MAX_PATIENTS && !waitingRoom.isEmpty()){                               //if treatment room has space move a patient from waiting to treatment
                Patient p = waitingRoom.peek();
                treatmentRoom.add(p);
                totalWait += p.getWaitingTime();
                if (p.getPriority() == 1)   {pri1TotalWait += p.getWaitingTime();}
                waitingRoom.remove();
            }
